...
def _element_to_tuple(
        qualifier_element: ElementTree.Element
        ) -> tuple:
    """
    ...
    name = __get_text(
        next(qualifier_element.iter(tag=f"{NS}name"), None)
        )
    group_id = __get_text(
        next(qualifier_element.iter(tag=f"{NS}groupId"), None)
        )
...
def get_qualifiers(
        url: str, verify: bool = False, logger=LOGGER
        ) -> pd.DataFrame:
    """
